...
public class BottomTabsLayout extends BaseLayout implements AHBottomNavigation.OnTabSelectedListener {
    ...
    private void setInitialTabIndex() {
        int position = AppStyle.appStyle.bottomTabsInitialIndex;
        bottomTabs.setCurrentItem(position);

        // set Selected icon
        ScreenParams currentScreenParams = params.tabParams.get(position);
        if (currentScreenParams.selectedTabIcon != null){
            bottomTabs.getItem(position).setDrawable(currentScreenParams.selectedTabIcon);
        }
    }
    ...
    // here we will update icon based on selectedIcon param
    @Override
    public boolean onTabSelected(int position, boolean wasSelected) {
        ...
        // check if current bottomTab has selectedIcon, then set it
        ScreenParams currentScreenParams = params.tabParams.get(position);
        if (currentScreenParams.selectedTabIcon != null){
            bottomTabs.getItem(position).setDrawable(currentScreenParams.selectedTabIcon);
        }

        // check if previous bottomTab has selectedIcon then reset to tab icon
        ScreenParams previousScreenParams = params.tabParams.get(unselectedTabIndex);
        if (previousScreenParams.selectedTabIcon != null){
            bottomTabs.getItem(unselectedTabIndex).setDrawable(previousScreenParams.tabIcon);
        }
